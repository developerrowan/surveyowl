...
import { ActionIcon, Alert, Box, Button, Center, Checkbox, Container, CopyButton, Divider, Group, Input, Loader, LoadingOverlay, NumberInput, Radio, Stack, Table, Text, TextInput, Textarea, Title, Tooltip, Transition } from '@mantine/core';
...
export default function SurveyView({ params, searchParams }: { params: { id: string, managerId: string }, searchParams: { fromCreation: boolean } }) {
    ...
    useEffect(() => {
        ...
        survey.questions.forEach(question => {
            const newDisplay: any = { question, totalResponses: 0, totalResponsesAcrossSurveys: 0, responses: [] };
        ...
        responses.forEach(res => {
            res.answers.forEach(answer => {
                const display = questionDisplay.find(x => x.question.id === answer.questionId);

                answer.values.forEach(val => {
                    if (display) {
                        if (val.length !== 0)
                            display.totalResponses += 1;
    ...
    const generatePdf = () => {
        html2canvas(reportTemplateRef.current as unknown as HTMLElement).then((canvas) => {
                        ...
                        <Stack justify="center" align="center">
                            ...
                            <Group>
                                <Link href={`/${params.id}/${params.managerId}`}>
                                    <Button variant="filled" size="md">Go back</Button>
                                </Link>
                                <Button color="red" size="md" onClick={generatePdf}>Export to PDF</Button>
                            </Group>
                        </Stack>
                    </Center>
                    <Center>
