...
import { Box, Button, Center, Checkbox, Container, Divider, Group, Input, Loader, LoadingOverlay, NumberInput, Radio, Stack, Text, TextInput, Textarea, Title, Transition } from '@mantine/core';
            ...
            switch (question.type) {
                ...
                case 'textarea':
                    if (question.required)
                        resolver = z.string().min(1, { message: 'This field is required.' })
                    else
                        resolver = z.string().min(1, { message: 'This field is required.' }).optional().or(z.string().length(0));
                    break;
                case 'email':
                    if (question.required)
                        resolver = z.string().min(1, { message: 'This field is required.' }).email('Please enter a valid email address.')
                    else
                        resolver = z.string().email('Please enter a valid email address.').optional().or(z.string().length(0));
                    break;
                case 'phone':
                    if (question.required)
                        resolver = z.string().regex(/^(\([0-9]{3}\) |[0-9]{3}-)[0-9]{3}-[0-9]{4}$/, 'Please enter a valid phone number')
                    else
                        resolver = z.string().regex(/^(\([0-9]{3}\) |[0-9]{3}-)[0-9]{3}-[0-9]{4}$/, 'Please enter a valid phone number').optional().or(z.string().length(0))
                    break;
                case 'number':
                    if (question.required)
                        resolver = z.number()
                    else
                        resolver = z.number().optional().or(z.coerce.string().length(0));
                    break;
                case 'multiselect':
                    if (question.required)
                        resolver = z.array(z.string()).min(1, 'Please select at least one response.');
                    else
                        resolver = z.array(z.string()).min(1, 'Please select at least one response.').optional().or(z.array(z.string()).length(0));
                    break;
                case 'radio':
                    if (question.required)
                        resolver = z.string().min(1);
                    else
                        resolver = z.string().min(1).optional().or(z.string().length(0));
                    break;
                default:
                    resolver = z.string();
                    break;
    ...
    const submitForm = () => {
        ...
        survey.questions.forEach(question => {
            ...
            questionResponses.push({
                ...
                values: Array.isArray(responses) ? responses : [responses ? responses.toString() : '']
            });
        });
